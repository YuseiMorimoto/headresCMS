...
- `docs/10` の要確認事項 R1〜R8 は**仮の前提**で計画している。R1（管理画面の許可）と R5（承認公開の文言）は `AGENTS.md` の改訂を伴うため、PR-01 で運営者の承認を得てから段階1に着手する
...
flowchart LR
  ...
  WF --> GSC
  WF --> Official
  WF -->|承認後のみ commit| Repo --> Deploy --> Static
  Deploy -->|sync-links| KV
  Go --> KV
  Go --> AE
  WF -->|SQL API| AE
  Preview -->|Service Binding| Api
```
...
| 認証 | Cloudflare Access（One-time PIN、無料枠 50 ユーザー） | 自前ログインを書かない。Worker 側で `Cf-Access-Jwt-Assertion` を Access の公開鍵で検証し、`aud` と email を照合する（N01）。ユーザー登録機能は作らない |
| 運営コンソール UI | サーバーレンダリング HTML + `<form method="post">`。クライアント JS は 0KB を目標 | 新規 UI フレームワークを追加しない（`AGENTS.md`）。Astro を第2プロジェクトとして持ち込まない。テンプレートはタグ付きテンプレート関数（自動エスケープ）で実装 |
...
| プレビュー | 公開サイトの `/preview/` に `?v={版ID}` を追加し、Service Binding で ops の版データを取得。Access で保護 | F21「実サイトと同じ表示」。既存の microCMS `?id=&draftKey=` は残す |
...
```
worker/ops/
├── wrangler.jsonc            # D1 / R2 / KV(LINKS 読取) / Workflows / Cron / Access 設定
├── migrations/               # D1 マイグレーション
├── src/
│   ├── index.ts              # fetch（コンソール・API）/ scheduled（Cron）
│   ├── auth/access.ts        # Access JWT 検証・権限判定
│   ├── db/                   # 型付きクエリ（any 禁止）
│   ├── budget/               # 予約・精算・上限判定
│   ├── jobs/                 # Job 永続化・再試行分類
...
| PR-01 | 規約改訂の提案（**運営者承認が必要**） | `AGENTS.md`「やらないこと」から管理画面を除き「運営コンソール（別 Worker・Access 保護）は許可」を追記、「自動化は下書き作成までで停止する」を「運営者の承認操作なしに公開しない」に改訂。`docs/00` 非目標も同期 | R1, R5 | — |
| PR-02 | 運営 Worker 骨格 | `worker/ops/` 雛形、`wrangler.jsonc`（D1・R2・Workflows・Cron・`--env staging`）、Access JWT 検証、`/healthz`、自動化停止スイッチ、`deploy-ops.yml`、`vitest` 導入 | A03, N01, N05 | T14（未認証拒否） |
...
| PR-16 | プレビュー | `/preview/?v=` + Service Binding、Access で `/preview/*` 保護、内部リンク候補を公開済みに限定 | F18, F21, N01 | T05, T14 |
| PR-17 | 運営コンソール最小版 | 「今日の操作」、承認待ち一覧、詳細（要約・出典・検品結果・案件・価格条件・推定費用）、承認／修正依頼／保留／却下、修正版の差分表示 | §10, F21 | T05, T14 |
...
| PR-26 | コンソール詳細画面 | 案件一覧、記事履歴、出典、実行ログ、接続状態、予算と設定（B05・O01 の変更） | §10 | — |
...
| Cloudflare Access | 0 | 50 ユーザーまで無料 |
...
| Cloudflare Access のチーム名・許可メールアドレス | PR-02 | `wrangler secret`（`ACCESS_AUD`, `ACCESS_TEAM_DOMAIN`） |
