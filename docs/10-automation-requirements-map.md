...
| B05 | 未実装 | 週2本・修正2件・承認待ち5件の設定がない | 設定テーブル + 画面（§10） |
...
| F21 | 一部 | `/preview/` は microCMS の draftKey 前提。認証なし | 運営コンソール + 版ID指定プレビュー（公開サイトと同一レンダリング） |
...
| O09 | 一部 | GitHub Issue / Actions 通知 | 管理画面内通知を標準。外部通知は設定された宛先のみ |

## 9. 管理画面とデータ要件（§10, D01〜D06）

| ID | 状態 | 根拠 | 不足・対応 |
|---|---|---|---|
| §10 画面 | 未実装 | `AGENTS.md` と `docs/00` は管理画面を非目標としている | **要確認事項 R1**。公開サイトとは別 Worker の運営コンソールとして実装する前提で計画 |
...
| ID | 状態 | 根拠 | 不足・対応 |
|---|---|---|---|
| N01 | 一部 | 秘匿値は Secrets / `.dev.vars`（`AGENTS.md`）。`/preview/` は draftKey のみで認証なし | Cloudflare Access で運営コンソールと `/preview/` を保護。Worker 側で Access JWT を検証 |
...
| ID | 内容 | 影響 | 仮の前提（承認待ち） |
|---|---|---|---|
| R1 | `AGENTS.md`「やらないこと: 管理画面・ダッシュボードの実装」と §10 の運営画面必須が矛盾 | 段階1以降の全 PR | 公開サイトとは別 Worker の運営コンソールを許可する改訂を PR-01 で提案 |
...
| R5 | `AGENTS.md`「自動化は下書き作成までで停止する」と「承認して公開」ボタン | F23, F24 | 公開は運営者の明示操作でのみ実行される。文言を「運営者の承認操作なしに公開しない」に改訂 |
